...
**扱ってよい商材は「販売に資格が要るかどうか」では切らない。** アフィリエイトサイトは販売しないので、登録販売者も酒類販売業免許も古物商もこちらには掛からない。掛かるのは広告表現の側で、薬機法66条・健康増進法65条・景表法はいずれも「その商品が何か」ではなく「何と書いたか」で判定される。プロテインは販売資格が不要だが「疲労回復に効く」と書けば抵触しうる。

そこで **`fitness` / `beauty` / `makeup` は残し**、`EXPRESSION_RULES`（景表法・薬機法・健康増進法の3法令別）で表現を弾く。プロテインは掲載可（2026-09-06 のユーザー決定）。

- **偽陽性を出さない語選びが命**。全記事が落ちるゲートは誰も直さなくなる。「改善」「回復」「予防」の単体は日常語なので入れず、「症状が改善」「疲労回復」のように効能の主張になる形だけを拾う
- ⚠️ **`PROHIBITED_GOODS` に「医薬部外品」を入れてはいけない。** 薬用化粧水・薬用美容液の表示で楽天の商品名にそのまま入っており、初回実装では公開済み61本のうち3本（`makeup-primer`・美容液・化粧水）が誤検出で落ちた。医薬部外品で本当に危ないのは承認範囲を超えた効能表現なので、それは薬機法の表現リストが受け持つ
- 扱わない商材は医薬品・処方薬・医療機器・コンタクトレンズだけ。ここは商品名も見る（楽天から混ざって返ってきたら講評を書かせる前に落とす）
- テストは `python3 scripts/test_quality_check.py`（18件）。**公開済み61本が全て通ることを確認済み**（2026-09-06）
