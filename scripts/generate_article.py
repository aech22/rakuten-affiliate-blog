...
- **intro は結論から書く(3〜4文)。** 最初の1〜2文で「どれを誰に薦めるか」「読者は何を基準に選べばいいか」の答えを出し、
  そのあとに理由を続ける。背景説明・前置き・問いかけ（「〜ではないでしょうか」）から始めない。
  読者は intro のあとに比較表と各商品の講評を読むので、intro で先に結論を渡しておく。
- **一次体験を装わない。** 「使ってみた」「実際に試した」「愛用している」とは書かない。
  こちらは商品を購入も使用もしていないので、体験談を書くと事実と違う主張になる。
  具体を出すときは spec と用途から言える範囲で、想定される使用場面として書く。
- outro はまとめ(3〜4文・選び方の指針)。intro の結論と食い違わせない。
